...
from Intedata_C import Intedata_C
...
if __name__ == '__main__':
    n_train = len(Intedata_C(protein))
    split = n_train // 5
    for i in range(1):
        ...
        train_loader = DataLoader(Intedata_C(protein), sampler=train_sampler, batch_size=TRAIN_BATCH_SIZE)
        test_loader = DataLoader(Intedata_C(protein), sampler=test_sampler, batch_size=TEST_BATCH_SIZE)
        device = torch.device(cuda_name if torch.cuda.is_available() else "cpu")
        model = modeling().to(device)
        loss_fn = nn.BCELoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=LR)
        with open('./Datasets/circRNA-RBP/' + protein + '/cTrain1.txt', 'a') as f0, open('./Datasets/circRNA-RBP/' + protein + '/cTest1.txt', 'a') as f1:
